...
mod tests {
    ...
    fn test_combine_filter() {
        ...
        assert!(result.contains("\"a\": 1") && result.contains("\"b\": 3") && result.contains("\"c\": 4"));
        ...
        let template = env
            ...
            .unwrap();
        let result = template.render(minijinja::context!()).unwrap();
        assert!(result.contains("\"a\": 1") && result.contains("\"b\": 2") && result.contains("\"c\": 3"));
    }
    ...
    fn test_items2dict_filter() {
        ...
        let template = env
            .template_from_str("{{ [{'key': 'a', 'value': 1}, {'key': 'b', 'value': 2}] | items2dict }}")
            .unwrap();
        let result = template.render(minijinja::context!()).unwrap();
    ...
    fn test_flatten_filter() {
        ...
        // Test already flat
        let template = env
            .template_from_str("{{ [1, 2, 3] | flatten }}")
            .unwrap();
        let result = template.render(minijinja::context!()).unwrap();
    ...
    fn test_map_filter() {
        ...
        let template = env
            .template_from_str("{{ [{'name': 'alice', 'age': 25}, {'name': 'bob', 'age': 30}] | map('name') }}")
            .unwrap();
        let result = template.render(minijinja::context!()).unwrap();
    ...
    fn test_dict2items_items2dict_roundtrip() {
        ...
        let template = env
            ...
            .unwrap();
        let result = template.render(minijinja::context!()).unwrap();
        assert!(result.contains("\"a\": 1") && result.contains("\"b\": {\"nested\": 2}") && result.contains("\"c\": [3, 4]"));
    }
}